...
public class OwnerService {
    ...
    public OwnerDto updateOwnerDetail(Owner owner){
        Owner existingOwner=ownerRepo.findById(owner.getId()).orElseThrow(()-> new ResourceNotFoundException("owner is not found for this is : "+owner.getId()));
    ...
    public void deleteOwner(UUID ownerId){
        Owner owner=ownerRepo.findById(ownerId).orElseThrow(()-> new ResourceNotFoundException("owner details not found for this id : "+ownerId));
        siteRepo.deleteByOwner(owner);//because owner parent table che and site child table che so apde direct owner
        //delete kasu without deeting ownerSite so error apse ke first child table mathi remove karo
